...
extends kc {
    ...
    private void rebuildFilter() {
        ...
        String string = mkl.strip(this.searchText);
        for (int i = 0; i < mrk.Ar.size(); ++i) {
            mrk.Entry entry = mrk.Ar.get(i);
            if (!string.isEmpty() && !mkl.strip(entry.name).contains(string) && !mkl.strip(entry.desc).contains(string)) continue;
    ...
    public void aQ() {
        this.rebuildFilter();
        int n = (this.filteredIndices.size() + 3) / 4;
        if (this.Ab >= n) {
    ...
    protected void h(int n, int n2, int n3) {
        ...
        for (int i = n4 = this.Ab * 4; i < this.filteredIndices.size() && i < n4 + 4; ++i) {
            ...
            mrk.Entry entry = mrk.Ar.get(n6);
            int n7 = 36 + (i - n4) * 42;
            if (n >= this.width / 2 + 63 && n < this.width / 2 + 90 && n2 >= n7 + 22 && n2 < n7 + 34) {
                coord.Ar = entry.x;
                coord.As = entry.z;
                this.qI.a((kc)new dyn());
                return;
            }
            if (n >= this.width / 2 + 97 && n < this.width / 2 + 130 && n2 >= n7 + 22 && n2 < n7 + 34) {
                this.qI.a((kc)new mks(entry, n6, (kc)this));
                return;
            }
            if (n < this.width / 2 + 135 || n >= this.width / 2 + 165 || n2 < n7 + 22 || n2 >= n7 + 34) continue;
            mrk.Ar.remove(n6);
            mrk.At();
            this.aQ();
            return;
    ...
    public void a(int n, int n2, float f) {
        ...
        this.a(this.pX, rd.gH().al("rumo.markersList") + " (" + n4 + ")" + (n5 > 1 ? " " + rd.gH().al("rumo.page") + " " + (this.Ab + 1) + "/" + n5 : ""), this.width / 2, n6 + 18, 0xFFFFFF);
        for (int i = n3; i < n4 && i < n3 + 4; ++i) {
            int n7 = this.filteredIndices.get(i);
            mrk.Entry entry = mrk.Ar.get(n7);
            int n8 = 36 + (i - n3) * 42;
            this.d(this.width / 2 - 160, n8, this.width / 2 + 160, n8 + 40, -1610612736);
            this.d(this.width / 2 - 160, n8 + 40, this.width / 2 + 160, n8 + 41, -1342177281);
            String string2 = entry.importance == 1 ? "\u00a7l" : (entry.importance == 2 ? "\u00a7o" : "\u00a7n");
            this.b(this.pX, string2 + entry.name, this.width / 2 - 150, n8 + 3, entry.color);
            this.b(this.pX, "X:" + (int)entry.x + " Z:" + (int)entry.z + (entry.desc.isEmpty() ? "" : " | \u00a7o" + (entry.desc.length() > 30 ? entry.desc.substring(0, 30) + ".." : entry.desc)), this.width / 2 - 150, n8 + 14, 0x808080);
            this.b(this.pX, rd.gH().al("rumo.go"), this.width / 2 + 67, n8 + 26, 0xE0E0E0);
            this.b(this.pX, rd.gH().al("rumo.edit"), this.width / 2 + 100, n8 + 26, 0xE0E0E0);
            this.b(this.pX, rd.gH().al("rumo.del"), this.width / 2 + 135, n8 + 26, -23296);
        }
        if (n4 == 0) {
